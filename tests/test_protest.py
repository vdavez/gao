...
def protest_with_decision():
    with open('tests/data/test.html', 'r') as f:
        ...
        # this to collect *all* of the protests and create Protest objects.
        protest = etree.tostring(
                    tree.find_class('docketSearch')[49]
                  ).decode('utf-8')
    return protest
...
def test_protest_get_opinion(protest_with_decision):
    ...
    assert "The RFP, issued on April 17, 2014, contemplated the award of a cost-plus-fixed-fee level-of-effort contract for a base year and four 1-year options." in p.data["opinion"].decision
